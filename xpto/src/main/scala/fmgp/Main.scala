...
object Utils {
  ...
  def updateFunction(model: Object3D) = {
...
object Main { //extends JSApp {
  ...
  def init(scenario: Int) = {
    ...
    worldToRender.foreach { w =>
      val dimensions: proto.Dimensions.D = proto.Dimensions.D3
      val model = w //w.modelObject3D
      Global.camera =
        Some(Utils.newCamera(dom.window.innerWidth, dom.window.innerHeight))
      ...
      Global.controls = Global.camera.map { c =>
        ...
        dimensions match {
          ...
          case _ => //None
            Global.modelToAnimate = Some(model) //if (w.dimensions.isD3) Some(model) else None
        }
  ...
  val animate: js.Function1[Double, Unit] = (d: Double) => {
    Global.modelToAnimate.foreach(model => Utils.updateFunction _)
